...
public class JpaResultMapper extends ResultMapper {
    ...
    public <T> List<T> list(Query q, Class<T> clazz,int constructorIndex)throws IllegalArgumentException {
        ...
        Optional<Constructor<?>> ctorOp = Arrays.stream(clazz.getDeclaredConstructors())
        		.filter(con -> con.getAnnotation(NativeResultConstructor.class)!=null)
		        .filter(con ->constructorIndex==con.getAnnotation(NativeResultConstructor.class).index())
		        .findFirst();
        
        if(!ctorOp.isPresent()) {
        	 throw new RuntimeException("No constructor annotated with  NativeResultConstructor and index :"+constructorIndex);
        }
